...
export async function resolveInstagramContactAndLead(
  ...
  // "Instagram Contact" placeholder) OR its stored avatar_url is a Meta
  // CDN URL that is already expired or close to it (self-heal: repairs a
  // stale avatar for an already-named contact on its next inbound
  // message, with no bulk backfill needed — see meta-avatar-url.ts's
  // shouldRefreshMetaAvatar). This is NOT "is the avatar Meta-hosted" — a
  // freshly Meta-issued URL is still Meta-hosted but has a far-future
  // expiry, so it correctly does NOT re-trigger this block on the very
  // next message; only a URL nearing/past its own oe expiry does. Never
  // re-fetched at all once BOTH a real name AND a fresh (or non-Meta)
  // avatar are already in place. Never overwrites a human-edited name —
