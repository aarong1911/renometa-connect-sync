...
export async function resolveMessengerContactAndLead(
  ...
  // its stored avatar_url is still a raw, expiring Meta CDN URL from
  // before meta-avatar-cache.ts existed (self-heal path: repairs a
  // stale avatar even for an already-named contact, without a bulk
  // backfill). Never re-fetched on every message once BOTH a real name
  // AND a non-Meta-CDN avatar are in place — at that point neither
  // condition holds, so this block is skipped entirely on every
  // subsequent inbound message. Never overwrites a human-edited name
  // (Part 5/6) — see the patch logic below, which still gates
  // `full_name` on isPlaceholderName independently of why enrichment ran.
  ...
  const cachedAvatarUrl = metaProfile?.profilePic
    ? await cacheMetaAvatar(supabaseAdmin, {
        orgId, channel: "messenger", providerSenderId: senderId, remoteUrl: metaProfile.profilePic,
      })
    : null;
