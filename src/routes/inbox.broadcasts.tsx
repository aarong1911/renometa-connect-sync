...
import {
  ...
  Clock, CheckCircle2, FileEdit, Plus, Sparkles, Tag,
} from "lucide-react";
import {
  useSegments, getSegment, useBroadcasts, createBroadcast, deleteBroadcast,
...
function BroadcastsPage() {
  ...
  return (
    <>
      <PageHeader
        title="Broadcasts"
        subtitle="Send one-to-many email and SMS campaigns to recipient segments"
        actions={
          <Button
      ...
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4 mb-6">
        <MetricCard icon={Megaphone} label="Sent" value={stats.sentCount} sub={`${stats.scheduled} scheduled · ${stats.drafts} drafts`} tone="primary" />
        <MetricCard icon={Users} label="Recipients reached" value={stats.totalRecipients.toLocaleString()} sub="across all sent broadcasts" tone="violet" />
        <MetricCard icon={Eye} label="Avg open rate" value={`${stats.openRate}%`} sub={`${stats.clickRate}% click · ${stats.replyRate}% reply`} tone="info" />
        <MetricCard icon={Tag} label="Segments" value={segments.length} sub="Available recipient segments" tone="muted" />
      </div>
...
const BROADCAST_STATUS_META: Record<Broadcast["status"], { label: string; tone: BadgeTone; icon: React.ComponentType<{ className?: string }> }> = {
...
function BroadcastStatusBadge({ status }: { status: Broadcast["status"] }) {
  const cfg = BROADCAST_STATUS_META[status];
  return (
