  ...
  return (
    <div className="space-y-4">
      <PageHeader
        ...
        title="AI Center"
        subtitle="Autonomous agents, on-demand AI tools, and voice agents — all in one place."
        actions={
      ...
      <Tabs value={topTab} onValueChange={handleTopTabChange}>
        ...
        <TabsContent value="agents" className="mt-4 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
            <MetricCard label="Active agents" value={loading ? "…" : stats.active} icon={Activity} tone="success" />
            <MetricCard label="Runs this week" value={loading ? "…" : stats.runs.toLocaleString()} icon={Zap} tone="primary" />
            <MetricCard label="Hours saved" value={loading ? "…" : `${stats.hours.toFixed(1)}h`} icon={Clock} tone="warning" />
            <MetricCard label="Avg success rate" value={loading ? "…" : `${stats.avgSuccess}%`} icon={TrendingUp} tone="success" />
          </div>
