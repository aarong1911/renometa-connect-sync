...
function mapRow(row: any, contactMap: Record<string, any>): Deal {
  ...
  return {
    ...
    // `stage_id` isn't updated when a deal is won (see updateDeal below —
    // only "lost" and normal stage moves touch stage_id), so a won deal's
    // stageSlug is stale (still whatever stage it was in right before
    // winning). Force "won" here the same way "lost" already is, or every
    // won deal silently stays bucketed under its pre-win stage everywhere
    // that reads `.stage` (Command Center pipeline grouping, the Pipeline
    // page's own `d.stage === "won"` column filter, etc).
    stage: row.status === "lost" ? "lost" : row.status === "won" ? "won" : stageSlug,
