...
export function addStage(pipelineId: string) {
  write(
    read().map((p) => {
      ...
      // Cycle through the palette by stage count so newly-added stages
      // are visually distinct from each other instead of all defaulting
      // to the same "muted" gray — still user-editable afterward.
      const color = STAGE_COLORS[p.stages.length % STAGE_COLORS.length];
      return { ...p, stages: [...p.stages, { id: uid(), name: "New Stage", color }] };
    }),
  );
}
