...
export function ContactRelatedTab({
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      const [{ data: projectRows }, { data: estimateRows }, { data: invoiceRows }] =
        await Promise.all([
          supabase
            .from("projects")
  ...
  // Appointments now render via EntityAppointmentsPanel below (Phase
  // 10.3), which has its own real create action and empty state, so it is
  // deliberately excluded from this combined "no related records at all"
  // fallback — that fallback is only for the still read-only sections
  // (Deals/Projects/Estimates/Invoices).
  const hasAny =
        ...
        {loadingRelated ? (
          ...
          <>
            ...
            {/* Phase 10.3: real, org-scoped, org-shared appointments panel
                ...
                read-only list that had no create action and used the
                pre-migration schema's `service`/`address` columns only. */}
            <section>
