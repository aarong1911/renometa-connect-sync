...
// The Cmd/Ctrl+K listener below already handles both keys — this only
// controls which label is shown, so the badge always matches what a key
// press on the current platform actually does.
      ...
      <div className="flex-1 max-w-2xl mx-auto relative">
        ...
        >
          ...
          <kbd className="hidden items-center gap-0.5 rounded border border-border bg-background px-1.5 py-0.5 text-[10px] font-medium sm:inline-flex">
            {IS_MAC ? <><CommandIcon className="h-2.5 w-2.5" />K</> : "Ctrl K"}
          </kbd>
        </button>
      </div>
      ...
      <Dialog open={shortcutsOpen} onOpenChange={setShortcutsOpen}>
        <DialogContent className="sm:max-w-sm">
          ...
          <div className="space-y-2 text-sm">
            {[[IS_MAC ? "⌘K" : "Ctrl K", "Global search"], [IS_MAC ? "⌘/" : "Ctrl /", "Focus sidebar"], ["Esc", "Close dialog"]].map(([key, label]) => (
