  ...
  return (
    <>
      ...
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4 mb-6">
        <KpiCard icon={Megaphone} label="Sent" value={String(stats.sentCount)} sub={`${stats.scheduled} scheduled · ${stats.drafts} drafts`} />
        <KpiCard icon={Users} label="Recipients reached" value={stats.totalRecipients.toLocaleString()} sub="across all sent broadcasts" />
        <KpiCard icon={Eye} label="Avg open rate" value={`${stats.openRate}%`} sub={`${stats.clickRate}% click · ${stats.replyRate}% reply`} />
        <KpiCard icon={Tag} label="Segments" value={String(segments.length)} sub="Available recipient segments" />
      </div>
