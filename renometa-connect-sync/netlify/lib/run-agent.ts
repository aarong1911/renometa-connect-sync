...
import { seedAiCenter } from "../functions/lib/seed-ai-center";
...
export const handler: Handler = async (event) => {
  ...
  // 3. Skip if disabled (unless manual trigger explicitly requested)
  ...
  try {
    ...
    // 10. Compute cost (haiku: ~$0.25/M tokens; sonnet: ~$3/M input + $15/M output — rough)
    const costUsd = definition.model?.includes("sonnet")
...
async function fetchAgentData(
  ...
  const sources: any[] = definition.data_sources ?? [];
    ...
    switch (source.type) {
      ...
      case "estimates": {
        if (source.filter === "recent_similar") {
          ...
          const jobType = estimate?.job_type ?? null;
          let q = supabase
            .from("estimates")
            .select("id, job_type, total_amount, estimate_items(*)")
            .eq("org_id", orgId)
      ...
      case "voice_call": {
        ...
        if (callId) {
          ...
          // trigger_data might include caller_phone + transcript directly
          result.call = {
...
function buildPrompt(
  ...
  if (realData.leads?.length)
    parts.push(`Leads to process (${realData.leads.length} total):\n${JSON.stringify(realData.leads, null, 2)}`);
  if (realData.invoices?.length)
    parts.push(`Overdue invoices (${realData.invoices.length} total):\n${JSON.stringify(realData.invoices, null, 2)}`);
  if (realData.projects?.length)
    parts.push(`Active projects (${realData.projects.length} total):\n${JSON.stringify(realData.projects, null, 2)}`);
...
async function executeActions(
  ...
  if (has("send_sms")) {
    ...
    if (Array.isArray(parsed)) {
      ...
      const phone = realData.call?.caller_phone ?? (realData.lead?.phone as string | undefined);
      ...
      // Plain text response (Review Agent, Speed-to-Lead)
  ...
  if (has("update_lead_contacted") && realData.leads?.length) {
    ...
    const processedIds: string[] = Array.isArray(parsed)
      ...
      : [];
    const ids = processedIds.length > 0 ? processedIds : (realData.leads as any[]).map((l) => l.id);
    for (const id of ids) {
      await supabase.from("leads").update({ last_contacted_at: now }).eq("id", id);
    }
    taken.push({ type: "leads_contacted_updated", count: ids.length });
  }

  // ── insert_note ────────────────────────────────────────────────────────────
  if (has("insert_note")) {
    // Qualification summary → lead ai_notes already handled above
    // Estimate review → project_notes on the estimate's project
    if (parsed && realData.estimate?.project_id) {
      const noteBody = JSON.stringify(parsed, null, 2);
      await supabase.from("project_notes").insert({
        project_id: realData.estimate.project_id,
        body: `AI Estimate Review:\n${noteBody}`,
        author: `AI - ${definition.name}`,
      });
      taken.push({ type: "note_inserted", preview: `Estimate review score: ${parsed.overall_score ?? "—"}` });
    }

    // Inbox triage → insert note summaries for urgent items
    ...
    for (const inv of realData.invoices as any[]) {
      if (inv.days_overdue > 30) {
        ...
        await supabase.from("tasks").insert({
          org_id: orgId,
          title: `Escalate collections for ${contactName} (Invoice #${inv.invoice_number ?? inv.id.slice(0, 8)})`,
          priority: "high",
          status: "todo",
        });
  ...
  if (has("create_lead") && parsed?.is_new_lead) {
    const callerPhone: string | undefined =
      realData.call?.caller_phone ?? (realData.lead?.phone as string | undefined);
    if (callerPhone) {
  ...
  if (has("create_estimate") && parsed?.line_items?.length) {
    const jobType = (realData as any).job_type ?? "Unknown";
